CREATE OR REPLACE FUNCTION almac_crm_completasalida(p_inout_id character varying)
  RETURNS character varying AS
$BODY$ DECLARE
BEGIN 
	IF P_INOUT_ID IS NULL OR (SELECT M_INOUT_ID FROM M_INOUT WHERE M_INOUT_ID = P_INOUT_ID) IS NULL THEN
		RAISE EXCEPTION '%', 'EL DOCUMENTO NO EXISTE'; --OBTG:-20000--
	END IF; 

	IF (SELECT COUNT(*) AS C FROM M_INOUTLINE WHERE M_INOUT_ID = P_INOUT_ID) <= 0 THEN
		RAISE EXCEPTION '%', 'EL DOCUMENTO NO TIENE LINEAS'; --OBTG:-20000--
	END IF;
...
END; $BODY$
  ...
  COST 100;
ALTER FUNCTION almac_crm_completasalida(character varying)
  OWNER TO tad;
